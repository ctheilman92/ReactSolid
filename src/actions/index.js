                        ...
                        if (newState.RegisteredAccounts.includes(newState.SenderAddress)) {
                            return newState.AccountsCtrct.deployed().then(inst2 => {
                                return inst2.getUserDetails(newState.SenderAddress); }).then(detes => {
                                    ...
                                    newState.SenderType = (detes[1] === true) ? 'VENDOR' : 'USER';
                                    newState.isRegisteredUser = true;
                                    newState.web3.eth.getBalance(newState.SenderAddress, (error, result) => {
                                        if (!error) {
                                            console.log('YOUR BALANCE: ' + result);
                                            newState.SenderBalance = result
                                        }
                                        else { console.log('error grabbing account balance') }
                                    });
                                        ...
                                        userPayments.map(p => {
                                            newState.AccountsCtrct.deployed().then(inst3 => {
                                                return inst3.getPaymentDetails(p); }).then(detail => {
                                                    ...
                                                    let otherAddress = (newState.SenderAddress === detail[2]) ? detail[3] : detail[2];
                                                    newState.AccountsCtrct.deployed().then(inst4 => {
                                                        console.log('OTHER ADDRESS ON LOADWEB3: ' + otherAddress)
                                                        return inst4.getUserDetails(otherAddress).then(oad => {
                                                            let pmntObject = { 
                                                                paymentHash: p, 
                                                                timeStamp: detail[0], 
                                                                amount: detail[1],
                                                            ...
                                                            newState.SenderPayments.push(pmntObject);
                                                            pmntsProcessed++;
...
export const addPaymentToUser = (inputAddress, amount, memo) => {
    return (dispatch, getState) => {
        ...
        acctp.then(inst => { 
            ...
            let amtToWei = currState.web3.toWei(amount, 'ether');
            let billTo, payTo;
            ...
            return inst.addPaymentToUser(billTo, payTo, amtToWei, memo, paymentHash, {from: currState.SenderAddress}).then(() => {
                return acctp.then(inst2 => {
                    return inst2.getPaymentDetails(paymentHash).then(dete => {
                        let otherAddress = (currState.SenderAddress === billTo) ? payTo : billTo;
                        
                        //try to get the name on the other account
                        acctp.then(inst3 => {
                            return inst3.getUserDetails(otherAddress).then(oad => {
                                //payment json object
                                let pmntObject = { 
                                ...
                                currState.SenderPayments.push(pmntObject);
...
export const processPayout = (pmnt) => {
    return (dispatch, getState) => {
        let currState = getState().accounts;
        dispatch({ 
            type: 'PROCESS_PAYOUT_REQUEST',
            payload: pmnt.paymentHash
        });

        let acctp = currState.AccountsCtrct.deployed();
        acctp.then(inst => {
            return inst.payout(pmnt.paymentHash, {from: currState.SenderAddress, value: pmnt.amount}).then(res => {
                //if success we can try to update the existing payment record in redux store
                //we can exclude the current saved payments that don't match the payment Hash
                let currPmnts = currState.SenderPayments.filter(statepmnt => {
                    return statepmnt.paymentHash !== pmnt.paymentHash
                });

                //we'll set this to as the new currState.SenderPayments then push the updated payment record
                let updatePmnt = currState.SenderPayments.filter(statepmnt => {
                    return statepmnt.paymentHash === pmnt.paymentHash
                });
                updatePmnt[0].isPaid = true;
                console.log('UPDATED PAYMENT RECORD: ' + util.inspect(updatePmnt))

                //first push previous payments to currState --- then push updated payment record to currState too.
                currState.SenderPayments = currPmnts;
                currState.SenderPayments.push(updatePmnt[0]);
                console.log('RETURNED SENDER PAYMENTS LIST: ' + util.inspect(currState.SenderPayments))
                return dispatch({
                    type: 'PROCESS_PAYOUT_SUCCESS',
                    payload: currState
                })
            }).catch((error) => {
                console.log('ERROR: ' + error)
